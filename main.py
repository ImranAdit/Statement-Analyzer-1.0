...
FRONTEND_ORIGIN = os.getenv("FRONTEND_URL", "")   # e.g. https://statement-analyzer-frontend.onrender.com

app.add_middleware(
    CORSMiddleware,
    allow_origins=[o for o in [FRONTEND_ORIGIN, "http://localhost:5173", "http://localhost:3000"] if o],
...
# This MUST be the backend URL (where /auth/callback lives)
BACKEND_URL          = os.getenv("BACKEND_URL", "http://localhost:8000")
# After login success, redirect user back to the frontend
...
async def login(request: Request):
    # Callback always points at the BACKEND so Google can POST the code here
...
# ── Serve React SPA (only when frontend/dist exists — i.e. unified deployment) ─
